...
/* syscall_handler - The main system call interface
 */
...
/* half - power_off()를 호출하여 핀토스를 종료한다.
...
/* exit - 현재 사용자 프로그램을 종료하고, 커널에 status를 반환한다.
 * 프로세스의 부모가 wait 시에 반환하는 status이다.
 * 일반적으로 0 상태는 성공을 나타내고 0이 아닌 값은 오류를 나타낸다.
 */
void exit(int status) {
 ...
 * 자식 프로세스의 pid를 반환해야 하며, 그렇지 않으면 유효한 pid가 아니어야 한다.
...
/* exec - 주어진 인수를 전달하여 현재 프로세스를 cmd_line에 지정된 이름의 실행 파일로 변경한다.
 * 성공하면 절대 반환되지 않는다.
 * 그렇지 않으면 어떤 이유로든 프로그램을 로드하거나 실행할 수 없는 경우, 프로세스가 status -1로 종료된다. 
 * 이 함수는 실행을 호출한 스레드의 이름을 변경하지 않는다.
 * 실행 호출이 진행되는 동안 fd는 열린 상태로 유지된다.
 */
int exec(const char *cmd_line) {
...
/* wait - 자식 프로세스 pid가 종료될 때까지 기다렸다가 자식의 종료 상태를 반환한다.
 * 자식 프로세스가 exit()를 호출하지 않았지만 커널에 의해 종료된 경우(예: 예외로 인해 종료된 경우) -1을 반환해야 한다. 
 * 부모 프로세스가 이미 종료된 자식 프로세스를 기다리는 것은 가능하지만, 
 * 커널은 부모가 자식의 종료 상태를 확인하거나 자식이 커널에 의해 종료되었음을 알 수 있도록 해야 한다.
 * 
...
/* create - initial_size 바이트 크기의 새 파일을 만들고 성공 여부를 반환한다. 
 * 새 파일을 만든다고 해서 파일이 열리지는 않는다.
 * 새 파일을 여는 것은 open 시스템 호출이 필요한 별도의 작업이다.
 */
bool create(const char *file, unsigned initial_size) {
	check_address(file);
...
/* remove - 파일을 삭제하고 성공 여부를 반환한다.
 * 파일은 열림 여부에 관계없이 제거할 수 있으며 열려 있는 파일을 제거해도 닫히지 않는다.
 * 열려있는 파일을 삭제해도 해당 파일에 대한 fd를 가진 모든 프로세스는 해당 파일을 계속 사용할 수 있다.
 * 즉, 파일에서 읽고 쓸 수 있다.
 * 파일에는 이름이 없으며 다른 프로세스가 파일을 열 수 없지만,
 * 모든 fd가 닫히거나 컴퓨터가 종료될 때까지 파일은 계속 존재한다.
 */
bool remove(const char *file) {
	check_address(file);
...
/* filesize - fd로 열린 파일의 크기를 바이트 단위로 반환한다.
 */
int filesize(int fd) {
 ...
 * 실제로 쓰여진 바이트 수를 반환하며, 일부 바이트가 쓰여지지 않은 경우 size보다 작을 수 있다. 
 * 파일 끝을 지나서 쓰면 일반적으로 파일이 확장되지만,
 * 현재 기본 파일 시스템에서는 파일 확장이 구현되지 않는다.
 * 
 * 예상되는 동작은 파일 끝 부분까지 가능한 한 많은 바이트를 쓰고 실제 쓰여진 바이트 수를 반환하거나,
 * 전혀 쓸 수 없는 경우 0을 반환하는 것이다.
 * 콘솔에 쓰는 코드는 적어도 크기가 수백 바이트보다 크지 않은 한 putbuf() 호출 한 번으로 모든 버퍼를 써야 한다. 
 * (큰 버퍼는 분할하는 것이 합리적이다)
 * 그렇지 않으면 다른 프로세스에서 출력한 텍스트 줄이 콘솔에 인터리빙되어 읽는 사람과 채점 스크립트 모두를 혼란스럽게 만들 수 있다.
 */
int write(int fd, const void *buffer, unsigned size) {
	check_address(buffer);
	if (fd == STDIN_FILENO) {
...
/* seek - 열린 파일 fd에서 읽거나 쓸 다음 바이트를 파일 시작부터 바이트 단위로 표시되는 position으로 변경한다.
 * (따라서 위치가 0이면 파일의 시작이다.) 
 * 파일의 현재 끝을 지나서 찾는 것은 오류가 아니다.
...
/* tell - 열린 파일 fd에서 읽거나 쓸 다음 바이트의 위치를 파일 시작 부분부터 바이트 단위로 반환한다.
 */
unsigned tell(int fd) {
	struct file *_file = get_file_from_fd(fd);
...
/* close - fd를 닫는다.
 * 프로세스를 exit하거나 terminate하면 열려 있는 모든 fd가 닫혀야 한다.
 */
void close(int fd) {
	struct file *_file = get_file_from_fd(fd);
	if (_file == NULL) {
